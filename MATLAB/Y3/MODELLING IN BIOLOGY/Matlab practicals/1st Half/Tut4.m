...
[t,v] = ode45(@(t,v) genswitch(v,a,b,g),tspan,[1,2]);
...
x = 0:0.01:4;
y = 0:0.01:4;
fy = a./(g*(1+y.^4));%for dx/dt
fx = b./(g*(1+x.^4));% for dy/dt
figure
hold on
grid on
...
[t,v] = ode45(@(t,v) genswitch(v,a,b,g),tspan,[2,1]);
...
plot(sol(3,1),sol(3,2),'k-o','linewidth',2); %Solution 3
legend('Nullcline for x', 'Nullcline for y','Solution 1', 'Solution 2','Solution 3','AutoUpdate','off')
x0r = 3*rand([80 2]); %so that all the values are ot only between 0 and 1
for i = 1:size(x0r,1)
    [t,v] = ode45(@(t,v) genswitch(v,a,b,g),tspan,x0r(i,:));
